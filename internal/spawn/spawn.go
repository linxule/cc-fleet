...
// SIGTERM/grace/SIGKILL cycle against any /proc cmdline that matches
...
// dependency edge. Teardown still runs its own reap on the normal
// teardown path; this is only the rollback-on-failure cleanup.
func defaultReapAgentProcess(agentID string) {
	...
	for _, pid := range pids {
		// Best-effort SIGTERM then short-grace then SIGKILL. Keep the grace
		// tight (200ms) so the fail-fast spawn JSON envelope stays timely.
