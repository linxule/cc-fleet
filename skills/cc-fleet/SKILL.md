...
- **`references/workflow.md`** — the workflow runtime: a JavaScript script (`cc-fleet workflow run`) run OFF this context that fans out vendor subagents via `const meta`/`agent`/`parallel`/`pipeline`/`phase`/`log` with native Workflow-API shape (await/Promise), an enforced pool + board run-tree; `schema` rides claude's native forced-StructuredOutput mechanism (client validation is a practical-subset backstop).
